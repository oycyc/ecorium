...
const barAvatar = {
  ...
  afterDatasetDraw(chart, args, options) {
    ...
    for (let i = 0; i < dataPoints.length; i++) {
      ctx.drawImage(
        avatarArray[i],
        x.getPixelForValue(i) - 15,
        y.getPixelForValue(dataPoints[i]) - 30,
        30,
        30
      );
    }
...
$("img[data-enlargeable]")
  ...
  .click(function () {
    ...
    modal = $("<div>")
      .css({
        /*modal with menu picture and styling*/
...
function countdown() {
  ...
  function addNumber() {
    ...
    if (num > 0) {
      ...
      time = time + num;
      document.querySelector("#travel_dist").innerHTML =
        time / 10000 + "<span>miles</span>";
      bg = bg + num * 0.25;
